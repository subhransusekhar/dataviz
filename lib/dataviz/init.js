var DataViz = {
	...
	drawVisualization: function() {
		  // To see the data that this visualization uses, browse to
			DataViz.query = new google.visualization.Query('http://spreadsheets.google.com/tq?key=pCQbetd-CptGXxxQIG7VFIQ&range=B1:C11&pub=1');
	...
	handleQueryResponse: function(response) {
		...
		visualization_geomap.draw(data, {
				"title": "Statewise IP Data"
			}
		);
		
		DataViz._initialize_map(data, true);
	...
	_get_lat_lon: function(data, map) {
		var geo_index = DataViz._get_lat_lon_index(data);
	...
	_get_lat_lon_index: function(data) {
		var lat_val = ['lat','latitude'];
		var lon_val = ['lon','longitude'];
		var geo_index = {};
		$.each(data.z, function(key, value) { 
			  if ($.inArray(value.id.toLowerCase(), lat_val) !== -1) {
				  geo_index['lat'] = key;
			  }
			  if ($.inArray(value.id.toLowerCase(), lon_val) !== -1) {
				  geo_index['lon'] = key;
			  }
		});
	...
	_initialize_map: function(data, cache) {
        ...
        if (cache)
			DataViz._get_lat_lon(data, map);
		else
			DataViz._get_lat_lon(DataViz._cache_data, map);
	...
	_populat_available_field: function() {
		
		$(".data_viz_available_field").each(function() {
			var first_option = $(this).html();
		...
		$('#data_component button').each(function(i) {
			if($(this).hasClass('active')) {
				...
				if($(this).hasClass('map')){
					$("#data_viz_map").show();
					DataViz._initialize_map(null, false);
				}
